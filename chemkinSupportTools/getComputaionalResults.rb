...
class Pressuremode
  ...
  def dataplot #Read premix.out -> Extract results only -> Plot data to csv file
    ...
    File.open(@resultfile, "r+") do |chem_l| #Serach number of end point of x
      ...
      #data output
      File.open("plot_#{@current_mixture}_#{@current_mech}_#{@current_condition}=#{@current_value}.txt", "w") do |prof_l|
        #l1 output
        prof_l.write "#{col_arr[1].split.slice!(0)} Tw[K] HRR[J/cm3sec] Density[g/cm3]  " #l1 output
        ...
        for eachline in 2..endpoint do
          if col_arr[eachline].split.slice!(1).to_f <= 2.5
            prof_l.write "#{col_arr[eachline].split.slice!(1)}  300  #{4.1868*col_arr[eachline].split.slice!(6).to_f} #{col_arr[eachline].split.slice!(5)}  "
          elsif col_arr[eachline].split.slice!(1).to_f >= 2.5 && col_arr[eachline].split.slice!(1).to_f < 4.5
            prof_l.write "#{col_arr[eachline].split.slice!(1)}  #{125*col_arr[eachline].split.slice!(1).to_f*col_arr[eachline].split.slice!(1).to_f-625*col_arr[eachline].split.slice!(1).to_f+1081.25} #{4.1868*col_arr[eachline].split.slice!(6).to_f}  #{col_arr[eachline].split.slice!(5)} "
          elsif col_arr[eachline].split.slice!(1).to_f >= 4.5 && col_arr[eachline].split.slice!(1).to_f < 6.5
            prof_l.write "#{col_arr[eachline].split.slice!(1)}  #{-125*col_arr[eachline].split.slice!(1).to_f*col_arr[eachline].split.slice!(1).to_f+1625*col_arr[eachline].split.slice!(1).to_f-3981.25}  #{4.1868*col_arr[eachline].split.slice!(6).to_f} #{col_arr[eachline].split.slice!(5)}  "
          else
            prof_l.write "#{col_arr[eachline].split.slice!(1)}  1300 #{4.1868*col_arr[eachline].split.slice!(6).to_f},#{col_arr[eachline].split.slice!(5)}  "
