...
class Sensitivityinput
  ...
  def initialize(inputfile = "chem.inp", outfile = "premix.out", resultfile = "result.txt", runfile = "run.bat", rateconstant_condition = 0, reaction_number = 0, main_directory = "Syngas5050_Keromnes_P=1.0", temporary_file = "temp.csv", locationrange = "10.0000", jump_makedb_signal = 1, make_reactionfile_signal = 1, failurelog = "failurelog.txt")
    @inputfile = inputfile
    @outfile = outfile
  ...
  def run_sensitivity
    ...
    if run
      ...
      puts "Failure. Please check #{@failure_file}"
    end
